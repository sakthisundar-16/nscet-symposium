        ...
        <Link to="/" className="flex items-center gap-3 group">
          ...
          <div className="hidden sm:block">
            ...
            <p className="text-[9px] text-[#1F2937] tracking-[0.18em] uppercase font-inter font-semibold mt-0.5">
              NSCET · Faculty of Computer Science & Engineering.
            </p>
          </div>
        </Link>

        {/* Desktop Nav */}
        <div className={`hidden lg:flex items-center gap-0.5 px-2 py-1.5 rounded-full transition-all duration-500 ${scrolled ? 'bg-transparent' : 'bg-white/60 backdrop-blur-md border border-[#C8845A]/18 shadow-sm'
          }`}>
          {navLinks.map(link => (
            <a key={link.name} href={link.path} onClick={(e) => handleNavClick(e, link.path)}
              className={`relative px-4 py-2 text-sm font-semibold rounded-full transition-all duration-200 font-inter ${isActive(link.path) ? 'text-[#A0623E]' : 'text-[#2D4A6A] hover:text-[#A0623E]'
                }`}
            >
              {isActive(link.path) && (
                <motion.span
            ...
            <div className="flex flex-col px-6 py-5 space-y-1">
              {navLinks.map((link, i) => (
                <motion.div key={link.name} initial={{ opacity: 0, x: -18 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06 }}>
                  <a href={link.path} onClick={(e) => handleNavClick(e, link.path)} className={`flex items-center px-4 py-3 rounded-xl text-base font-semibold transition-all duration-200 font-inter ${isActive(link.path)
